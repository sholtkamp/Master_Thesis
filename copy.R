...
#' @param tile_data Array containing numerical representation of raster data
#' @param mask_data Array containing class values for tile data
...
balance_data <- function(paradigm = "image", tile_data, mask_data, aim = 0.5){
  ...
  if(paradigm == "pixel"){
    ...
    print(paste0("Initial balance_score: ", balance_score))
...
# Build tensors 
tf_train_tiles <- k_constant(data[[1]][ , , , 4:5])
tf_train_masks <- k_constant(to_categorical(data[[2]], num_classes = 2))

tf_test_tiles <- k_constant(data[[3]][ , , , 4:5])
tf_test_masks <- k_constant(to_categorical(data[[4]], num_classes = 2))


tf_target_tiles <- k_constant(data[[5]][ , , , 4:5])
```

```{r}

model <- build_unet_model(input_shape = c(128, 128, 2), num_classes = 2)
...
results <- threshold_results(predictions, 0.80)

map <- superset_results(results, 10)
test <- superset_results(data[[5]][, , , 4], 10)


par(mfrow=c(1,2))
image(raster(test), main = "Input: NIR,RE, NIR shown")
