...
#'
#' @return Input data in an multidimensional array
#'
...
#' Prepare data for usage by reading tiles and masks 
#'
...
#'
#' @return Returns list of matrices, containing training and masks.
#' @export
#'
#' @examples \dontrun{data <- read_data(c(128, 128), 6)}
read_data <- function(dimensions = c(128, 128), channels = 6){
  ...
  read_data <- list("tile_data" = tile_data, "mask_data" = mask_data)
    ...
    for(j in 1:dim(temp_tiles)[2]){
      for(k in 1:dim(temp_tiles)[3]){
        for(l in 1:dim(temp_tiles)[4]){
          ...
          if(l == dim(temp_tiles)[4]) {
            temp_tiles[i, j, k, l] <- (data[i, j, k, 4] - data[i, j, k, 1]) / 
                                      (data[i, j, k, 4] + data[i, j, k, 1])
          }
        }
      }
    }
...
prep_data <- function(){
  ...
  #data <- balance_data("image", data, 0.5)
  
  cat("\nCalculating NDVI\n")
  data[[1]] <- add_ndvi(data[[1]])
  data[[3]] <- add_ndvi(data[[3]])
...
```{r data_generation}
# Build tensors 
tf_train_tiles <- k_constant(data[[1]][ , , , c(4,5,7)])
tf_train_masks <- k_constant(to_categorical(data[[2]], num_classes = 2))

tf_test_tiles <- k_constant(data[[3]][ , , , c(4,5,7)])
tf_test_masks <- k_constant(to_categorical(data[[4]], num_classes = 2))

tf_target_tiles <- k_constant(data[[5]][ , , , c(4,5,7)])


# Expand dimensions on 1D data 
#tf_train_tiles <- k_expand_dims(data[[1]][ , , , 7], -1)
#tf_test_tiles <- k_expand_dims(data[[3]][ , , , 7], -1)
#tf_target_tiles <- k_expand_dims(data[[5]][ , , , 7], -1)
...
model <- build_custom_model(input_shape = c(128, 128, 3), num_classes = 2)
...
pred <- model %>% predict(tf_target_tiles)

results <- threshold_results(pred, 0.7)
map <- superset_results(results, 10)
image(raster(map), main = "Predicted Classes, Threshold: 70%")
...
dropout mal rausnehmen
kreuzvalidierung durch auslassen
training vs val vs crossval fehler

dropout --> einfluss auf acc val
early stopping auf beides --> endet eins früher?

wenn val acc konstant overfitting

eine region komplett rauslassen --> nur für prediction
loop zum auslassen benutzen

vortrainierte weights testen, imagenet