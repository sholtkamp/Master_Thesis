...
#' Prepare data for usage by reading training data and masks, applying a training/test split to it and making it accessable via a variable
#'
...
prep_data <- function(dimensions = c(128, 128), channels = 6, split = 0.8){
  ...
  # enable script to run on both Windows and Linux for testing purposes by giving differnt path formats according to OS
  switch(Sys.info()[['sysname']],
         
         Windows= {
           tile_data <- read_input("E:/Master_Thesis/data/tiles/", dimensions, channels, ".tif")
           mask_data <- read_input("E:/Master_Thesis/data/masks/", dimensions, 1, ".tif")
         },
         
         Linux  = {
           tile_data <- read_input("/home/basti/Daten/Master_Thesis/data/tiles/", dimensions, channels, ".tif")
           mask_data <- read_input("/home/basti/Daten/Master_Thesis/data/masks/", dimensions, 1, ".tif")
         })
  ...
  # fill arrays with subsets of input data
  train_data <- tile_data[1:split_point, , , ]
  train_mask <- mask_data[1:split_point, , ,]
  train_mask[is.na(train_mask)] <- 0
  
  test_data <- tile_data[(split_point + 1):dim(tile_data)[1], , , ]
  test_mask <- mask_data[(split_point + 1):dim(mask_data)[1], , , ]
  test_mask[is.na(test_mask)] <- 0
  
  
  # make list of arrays to return
  prepped_data <- list("train_data" = train_data, "train_mask" = train_mask, "test_data" = test_data, "test_mask" = test_mask)
...
build_model <- function(input_shape = c(128, 128, 1), num_classes = 2) {
...
balance <- sum(data[[2]] != 0) / (dim(data[[2]])[1] * dim(data[[2]])[2] * dim(data[[2]])[3])
balance
