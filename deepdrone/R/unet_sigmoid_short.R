...
#'
#' @examples
build_unet_short <- function(input_shape = c(128, 128, 3), num_classes = 2){
  ...
  center <- down2_pool %>%
    ...
    layer_conv_2d(filters = 256, kernel_size = c(3, 3), name = "center_1", padding = "same") %>%
    layer_activation("relu") %>%
    layer_conv_2d(filters = 256, kernel_size = c(3, 3), name = "center_2", padding = "same") %>%
    layer_activation("relu")
