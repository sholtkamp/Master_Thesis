...
# load tile data
# data <- load_data()
# data <- deepdrone::bog_data
# load("F:/Master_Thesis/data/bog_data.rda")

# add NDVI to data 
# data[[1]] <- add_ndvi(data[[1]], 3, 4)
...
channels <- c(1, 2, 3)
load("F:/Master_Thesis/data/data.RData")
tensors <- tensorize_data(data, channels, c(1, 316, 328, 581, 617, 632, 712))
...
early_stopping_call = callback_early_stopping(monitor = "val_dice",
                                              mode = "max",
                                              min_delta = 0.0001,
                                              patience = 15,
...
for(i in 1:6){
  ...
  ####these.tiles <- k_concatenate(tile_tensors[c(-2)], 1)
  ####these.masks <- k_concatenate(mask_tensors[c(-2)], 1)
  
  cross_val <- loocv(i)
  these.tiles <- cross_val[1]
  these.masks <- cross_val[2]
